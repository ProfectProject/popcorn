...
## 스키마 컨벤션
- PK: `p_users.id` = BIGINT, 그 외 도메인 엔티티 = UUID
- FK: 참조 PK 타입과 동일 사용 (유저→BIGINT, 도메인→UUID)
- 시간 타입: `timestamp` 권장 (`created_at`/`updated_at` NOT NULL, `deleted_at` NULL 허용)
...
| created_by | BIGINT |  |  | 행 생성자(p_users.id, 운영자/시스템) |
...
| updated_at | timestamp |  |  | 수정 |
| deleted_at | timestamp |  |  | 삭제 |
| created_by | BIGINT |  |  | 생성자(스케줄/상품 담당) |
| updated_by | BIGINT |  |  | 수정자 |
| deleted_by | BIGINT |  |  | 삭제자 |
...
| schedule_option_id | UUID |  | FK(schedule_option_id → p_schedule_options.schedule_option_id) | 스케줄 옵션 ID |
| goods_variant_id | UUID |  | FK(goods_variant_id → p_goods_variants.goods_id) | 굿즈 옵션 ID |
| qty | int | YES |  | 수량 |
| unit_price | int | YES |  | 단가 |
| line_amount | int | YES |  | 금액 |
| created_at | timestamp |  |  | 생성 |
| updated_at | timestamp |  |  | 수정 |
| deleted_at | timestamp |  |  | 삭제 |
...
### 12. 주문 상태 이력 (`p_order_status_histories`)
> 주문 상태 변경 로그
