package com.popcorn.demo.domain.qr.checkin.controller;
...
@RestControllerAdvice(basePackages = "com.popcorn.demo.domain.qr.checkin.controller")
@Slf4j
public class CheckinExceptionHandler extends BaseController {
	...
	public ResponseEntity<BaseResponse<BaseError>> handleBaseException(BaseException ex) {
		log.warn("Checkin exception: code={}, message={}", ex.getResponseCode().getCode(), ex.getMessage());
	...
	public ResponseEntity<BaseResponse<BaseError>> handleGeneralException(Exception ex) {
		log.error("Checkin unexpected error: {}", ex.getMessage(), ex);
