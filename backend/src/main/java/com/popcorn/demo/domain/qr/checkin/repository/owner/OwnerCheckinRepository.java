package com.popcorn.demo.domain.qr.checkin.repository.owner;
...
public class OwnerCheckinRepository {
	...
	public List<CheckinRow> findByPopupId(UUID popupId, Long ownerId, int limit) {
		return jdbcTemplate.query(
				...
				FROM qr.checkins c
				JOIN qr.order_qr_codes q ON q.qr_id = c.order_qr_code_id
				JOIN "order".orders o ON o.order_id = c.order_id AND o.deleted_at IS NULL
				JOIN "order".order_goods og ON og.order_id = o.order_id AND og.deleted_at IS NULL
				...
				JOIN store.popups p ON p.popup_id = COALESCE(ps.popup_id, gv.popup_id) AND p.deleted_at IS NULL
				JOIN store.stores s ON s.store_id = p.store_id AND s.deleted_at IS NULL
				WHERE p.popup_id = ?
				  AND s.user_id = ?
	...
	public List<CheckinRow> findByScheduleId(UUID popupId, UUID scheduleId, Long ownerId, int limit) {
		return jdbcTemplate.query(
				...
				JOIN "order".orders o ON o.order_id = c.order_id AND o.deleted_at IS NULL
				JOIN "order".order_goods og ON og.order_id = o.order_id AND og.deleted_at IS NULL
				JOIN store.popup_schedules ps ON ps.schedule_id = og.schedule_id AND ps.deleted_at IS NULL
				JOIN store.popups p ON p.popup_id = ps.popup_id AND p.deleted_at IS NULL
				JOIN store.stores s ON s.store_id = p.store_id AND s.deleted_at IS NULL
