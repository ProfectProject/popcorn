...
export default function PaymentsClient() {
  ...
  useEffect(() => {
    ...
    const applyRedirect = (payload) => {
      if (!payload?.redirectUrl) {
        return;
      }
      const redirectUrl = new URL(payload.redirectUrl, window.location.origin);
      try {
        window.localStorage.removeItem("payment:success");
      } catch (err) {
    ...
    const handleStorage = (event) => {
      ...
      try {
        applyRedirect(JSON.parse(event.newValue));
      } catch (err) {
    ...
    const handleVisibility = () => {
      ...
      try {
        const value = window.localStorage.getItem("payment:success");
        if (value) {
          applyRedirect(JSON.parse(value));
        }
      } catch (err) {
        // ignore storage errors
      }
    ...
    const channel = "BroadcastChannel" in window ? new BroadcastChannel("payment") : null;
    const handleMessage = (event) => {
      if (event?.data?.type !== "payment:success") {
        return;
      }
      applyRedirect(event.data);
    };
  ...
  useEffect(() => {
    ...
    const resolvedOrderId = paymentInfo.orderId || paymentInfo.orderNo;
    if (!resolvedOrderId || !paymentInfo.amount) {
            ...
            <>
              ...
              {isDev && (
                <div className="card">
                  ...
                  <p className="mono">token: {token ? "ok" : "missing"}</p>
                  <p className="mono">scriptReady: {String(scriptReady)}</p>
                  <p className="mono">clientKey: {clientKey ? "set" : "missing"}</p>
                  <p className="mono">
                    orderId: {paymentInfo?.orderId ? "ok" : paymentInfo?.orderNo ? "fallback" : "missing"}
                  </p>
                  <p className="mono">paymentInfo: {paymentInfo ? "ok" : "missing"}</p>
