...
class QrExceptionHandlerTest {
	...
	void handleBaseException_returnsCorrectResponse() {
		...
		assertThat(response.getBody().getData().getMessage()).contains("팝업 굿즈 배송을 위해 배송지를 먼저 등록해주세요");
	}
	...
	void handleValidationException_returnsValidationError() {
		...
		assertThat(response.getBody().getData().getMessage()).contains("qrCode: qrCode는 필수입니다.");
	}
	...
	void handleBindException_returnsValidationError() {
		...
		assertThat(response.getBody()).isNotNull();
		assertThat(response.getBody().getCode()).isEqualTo(CommonResponseCode.INVALID_REQUEST.getCode());
		assertThat(response.getBody().getData().getMessage()).contains("field: 필드가 잘못되었습니다.");
	}
	...
	void handleTypeMismatchException_returnsTypeError() {
		...
		assertThat(response.getBody()).isNotNull();
		assertThat(response.getBody().getCode()).isEqualTo(CommonResponseCode.INVALID_REQUEST.getCode());
		assertThat(response.getBody().getData().getMessage()).contains("잘못된 String 형식입니다: invalid-uuid");
	}
	...
	void handleGeneralException_returnsInternalError() {
		...
		assertThat(response.getBody().getData().getMessage()).contains("일시적인 오류가 발생했습니다");
	}
	...
	void handleTypeMismatchException_withNullRequiredType() {
		...
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
		assertThat(response.getBody()).isNotNull();
		assertThat(response.getBody().getData().getMessage()).contains("잘못된 알 수 없는 타입 형식입니다: invalid-value");
	}
	...
	void handleValidationException_withNoFieldErrors() {
		...
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
		assertThat(response.getBody()).isNotNull();
		assertThat(response.getBody().getData().getMessage()).isEqualTo("입력값이 올바르지 않습니다.");
	}
